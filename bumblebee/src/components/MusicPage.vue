<template>
  <div>
    ...
    <div class="container mx-auto mt-24 p-6">
      <h2 class="text-3xl font-bold text-center mb-8">Song Data</h2>
  ...
  methods: {
    async fetchMusicData(page) {
      ...
      try {
        ...
        // Assign the data directly to musicData
        if (response.data && response.data.musicData) {
          ...
          this.musicData.forEach((item) => {
            if (item.id) {
              this.showLyrics[item.id] = false;
            }
          });

          console.log("Music Data Loaded:", this.musicData);
        } else {
          this.error =
