<template>
  <div>
    ...
    <div class="container mx-auto mt-24 p-6">
      ...
      <!-- Predicted Song Section (Displayed at Round 10) -->
      <div
...
export default {
  ...
  data() {
    return {
      gameId: null,
      username: "", // Username input for starting the game
      gameStarted: false,
      songs: [],
      favoriteSong: {},
      previousFavorite: {}, // Field for previous favorite
      historicalFavorite: {}, // Field for historical favorite
      leaderboard: [],
      error: null,
      loading: false,
      completed: false,
      currentRound: 1,
      totalRounds: 10,
      predictedSong: {}, // New property to store predicted song
    };
  },
  computed: {
    progressBarWidth() {
    ...
    async fetchRandomSongs() {
      try {
        ...
        if (response.data.songs.length < 2) {
          ...
          this.previousFavorite = response.data.lastFavorite || {}; // Set last favorite
          this.historicalFavorite = response.data.historicalFavorite || {}; // Set historical favorite
    ...
    async selectSong(selectedSongId, unselectedSongId) {
      try {
        ...
        if (response.data.completed) {
          ...
          this.previousFavorite = response.data.secondLastFavorite || {}; // Adjust to use secondLastFavorite
          this.historicalFavorite = response.data.historicalFavorite || {}; // Set historical favorite
